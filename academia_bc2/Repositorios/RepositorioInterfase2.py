...
class RepositorioInterface(Generic[T]):
        ...
        def __init__(self):
            certAuth = certifi.where()  # Autoridad certificadora
            dataConfig = self.loadFileConfig()  # datos de configuracion
            client = pymongo.MongoClient(dataConfig["bd=connection"], tlsCAFile=certAuth)  # cliente conectado a la db
            self.baseDatos = client[dataConfig["bd-name"]]  # Base de datos
